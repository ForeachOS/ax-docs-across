[#partial-rendering]
== Partial rendering
Apart from link:index.adoc#layout-templates[layout templates], AcrossWebModule also activates support for partial rendering: rendering only a specific fragment of a (Thymeleaf) template, or even a single `ViewElement`.
