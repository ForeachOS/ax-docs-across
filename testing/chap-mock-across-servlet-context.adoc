...
`MockAcrossServletContext` is used automatically behind the scenes when using the link:index.adoc#test-context-builders[test builders] or link:index.adoc#annotations[annotations].
In an integration test scenario, the implementation can either be wired as a bean (when using the link:index.adoc#annotations[annotations]) or retrieved from the resulting `AcrossTestWebContext`.
