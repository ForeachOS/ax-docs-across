...
|Utility bean that can be used to verify if link:../developing-applications/index.adoc#development-mode[development mode] is active, and to get development locations for resources files.

|`MessageSource`
|The default Across `MessageSource` bean that represents the module based message source hierarchy.
See the chapter on link:../developing-modules/index.adoc#message-sources[Message sources] for more information.
