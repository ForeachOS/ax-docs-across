...
|*runCondition*
|Condition when this installer should run.  See link:index.adoc#installer-run-conditions[run conditions].
...
This allows for installers to execute only if they have not yet been executed (see link:index.adoc#installer-run-conditions[run conditions]).
...
Because of tracking and synchronization, a link:index.adoc#installer-datasource[datasource] is required in order to execute installers.
...
Only beans from the parent `ApplicationContext` or the link:index.adoc#installer-applicationcontext[installer `ApplicationContext`] are available.
...
In addition to the standard run conditions, you can use link:index.adoc#installer-conditionals[`@Conditional` annotations on installer classes].
...
This allows grouping types of installers together (for example schema) and overruling their execution using link:index.adoc#installer-settings[`InstallerSettings`].

[discrete]
=== Installer execution order
If all conditions are fulfilled, installers will be executed in a predetermined order:

. in link:index.adoc#installer-bootstrap-phases[bootstrap phase] order
. in link:../developing-applications/index.adoc#module-bootstrap-order[module bootstrap order]
