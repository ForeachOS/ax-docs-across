...
Usually not necessary to use the `#webapp` however, see the link:index.adoc#url-support[URL support section].
...
[#url-support]
=== URL support
Apart from the availability of *#webapp*, AcrossWebModule activates transparent support for the link:index.adoc#web-app-path-resolver-and-path-prefixing[`WebAppPathResolver`] on Thymeleaf URL generation. +
