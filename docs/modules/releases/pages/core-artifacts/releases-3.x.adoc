...
* added support for link:{doc-spring-boot}#across-configuration[`META-INF/across.configuration`]
...
* several changes to the menu building infrastructure, some breaking. See the link:{doc-across-web}#menu-infrastructure[updated documentation] for use case examples.
...
** link:{doc-testing}#expose-for-test[`@ExposeForTest`] can be used in combination with `@SpringBootTest`
