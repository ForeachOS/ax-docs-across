...
=== Fundamentals
When creating an across application
...
This will initialize a new `AcrossContext` with the main `ApplicationContext` as parent, look for a link:index.adoc#across-datasource[default datasource bean] named *acrossDataSource* and then configure modules based on the `@EnableAcrossContext` attribute values.
...
For a module to be autoconfigurable it should adhere to certain conventions, see the section on link:index.adoc#creating-an-across-module[creating an Across module] for more details.
...
. Modules added through link:index.adoc#acrosscontextconfigurer[`AcrossContextConfigurer` instances]
